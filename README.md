...
| Fig 4 | `plot_fig4_scenarios.py` | Scenario comparison (A: stacked area, B: OT rate + correctly treated, C: per-pathway bars, D: cumulative avoided, E: avoided per test, F: cost curve) |
| Fig 5 | `plot_fig4_scenarios.py` | Net savings per test heatmaps (2×2 grid, one per use case) |
...
| `plot_bia_costcurve.py` | Standalone cost-curve figure (also generated by `plot_fig4_scenarios.py`) |
